# phase2_scoring_v3.py
# Pathology-focused Phase 2 (v3)
#
# Inputs:
...
if "bbb_score" in out.columns:
    out["phase2_score_v3"] = 0.7 * out["ad_score_gated"] + 0.3 * out["bbb_score"].fillna(0)
else:
    out["phase2_score_v3"] = out["ad_score_gated"]

out = out.sort_values("phase2_score_v3", ascending=False)
...
top = out.head(30)[["drug_name_out", "num_targets_moa", "num_core_hits", "ad_hit_targets", "phase2_score_v3"]]

with open("phase2_v3_report.txt", "w", encoding="utf-8") as f:
    f.write(f"Total BBB+ drugs: {len(out)}\n")
    nonzero = (out["phase2_score_v3"] > 0).sum()
...
print("✅ Saved phase2_v3_report.txt")
print("\nTop 30 candidates (v3):")
print(top)
